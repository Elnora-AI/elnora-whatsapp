...
Check for `git`, `go`, `uv`, `node`, `curl` (plus a C compiler on Linux, and
`gcc` on Windows — CGO is required for go-sqlite3). For anything missing,
offer the platform's install command (brew / apt / winget / MSYS2 for gcc)
and wait for the user's go-ahead before installing anything.
...
1. Tell the user to run in their own terminal:
   `bash "${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh"` (Windows, in PowerShell:
   `& "${CLAUDE_PLUGIN_ROOT}\scripts\pair.ps1"`) — a QR code appears; they
