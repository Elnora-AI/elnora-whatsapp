...
# 3. Permissions (auto-tighten): the bridge log contains full message
#    content, so it gets the same lockdown as the store.
[ -d "$INSTALL_DIR" ] && chmod 700 "$INSTALL_DIR" 2>/dev/null
[ -f "$LOG_PATH" ] && chmod 600 "$LOG_PATH" 2>/dev/null
if [ -d "$BRIDGE_DIR/store" ]; then
  ...
  ok "permissions (install dir + store 700; DBs, token, log 600)"
else
...
# 5. Bridge reachability + pairing state. The REST port only opens once a
#    WhatsApp session exists, so "unreachable + QR banner in the log" means
#    NOT PAIRED, not broken.
awaiting_qr() {
