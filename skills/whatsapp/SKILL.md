...
- Session dropped / needs re-pairing: run `scripts/pair.sh` in a terminal —
  it shows a QR code; scan within ~20s (WhatsApp > Settings > Linked
  Devices). If only the service log is available, render the QR from it:
