...
Check, in order: `git`, `go`, `uv`, `node`, `curl` (all on PATH). Linux only:
also a C compiler (`cc`/`gcc`/`clang` — CGO is required for go-sqlite3;
`apt install build-essential`). Windows only: `gcc` (same CGO reason; if
missing, point the user at MSYS2: `pacman -S mingw-w64-ucrt-x86_64-gcc`,
...
1. Preferred: the user runs `bash <plugin-root>/scripts/pair.sh` (Windows:
   `<plugin-root>\scripts\pair.ps1` in a PowerShell window) in their own
   terminal and scans the QR it prints (WhatsApp > Settings > Linked
   Devices > Link a Device). The script stops the service/task first and
   restarts it after, automatically.
